...
class _CreateArticleScreenState extends ConsumerState<CreateArticleScreen> {
  late ImageMarkerTextEditingController _controller;
  ...
  void initState() {
    super.initState();
    _controller = ImageMarkerTextEditingController(
      images: _images,
  ...
  Future<void> _pickImage() async {
    ...
    // 2️⃣ Mutate images OUTSIDE setState
    _images.add(filename);

    // 3️⃣ Defer rebuild safely
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) setState(() {});
    });
  ...
  Widget _buildContentWithImages(bool isDark) {
    ...
    for (final line in lines) {
      ...
      if (renderedImages.contains(index)) continue;
      renderedImages.add(index);

      if (index >= _images.length) continue;
  ...
  Widget _buildInlineEditor(bool isDark) {
    return EditableText(
